...
# Function to generate OSM_ID based on Latitude and Longitude
def generate_osm_id(lat, lon):
    return f'OSM_{int(lat*10000)}_{int(lon*10000)}'

# Apply the function to create OSM_ID column
filtered_data['OSM_ID'] = filtered_data.apply(lambda row: generate_osm_id(row['Latitude'], row['Longitude']), axis=1)

# Group by OSM_ID and calculate the number of accidents and accident severity scores
result = filtered_data.groupby('OSM_ID').agg(
    number_of_accidents=('OSM_ID', 'size'),
    accident_severity_scores=('Accident_Severity', lambda x: list(x))
).reset_index()

# Save the result to a CSV file
result.to_csv('accidents_summary.csv', index=False)
